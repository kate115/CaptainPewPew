	...
	constructor() {
		...
		eventEmitter.on(Messages.HERO_FIRE, () => {
			if (coolDown === 0) {
				...
				gameObjects.push(l);
				cooling();
        let pew = new Audio('PewPew.m4a');
        pew.play();
			}
		});
		eventEmitter.on(Messages.GAME_END_LOSS, (_, gameLoopId) => {
			game.end = true;
			displayMessage('You died?! We need a new captain, or press [ENTER] to try again.');
			clearInterval(gameLoopId);
		});

		eventEmitter.on(Messages.GAME_END_WIN, (_, gameLoopId) => {
			game.end = true;
			displayMessage('Now, you are the new captain. Good Job! Want a new wave? Press [ENTER]', 'green');
			clearInterval(gameLoopId);
		});
		...
		eventEmitter.on(Messages.COLLISION_MONSTER_HERO, (_, { monster: m, id }) => {
			...
			if (game.life === 0) {
				...
				gameObjects.push(new Explosion(hero.x, hero.y, laserRedShot));
			}
			hero.img = heroImgDamaged;
			m.dead = true;
...
window.onload = async () => {
	...
	heroImg = await loadTexture('spaceArt/player.png');
	monsterImg = await loadTexture('spaceArt/enemyShip.png');
	laserRedImg = await loadTexture('spaceArt/laserRed.png');
	laserRedShot = await loadTexture('spaceArt/laserRedShot.png');
	lifeImg = await loadTexture('spaceArt/life.png');
	...
	ctx.fillStyle = 'black';
	ctx.fillRect(0, 0, canvas.width, canvas.height);
	displayMessage('We need to train you to be the new captain. Press [ENTER] to start.', 'blue')
};